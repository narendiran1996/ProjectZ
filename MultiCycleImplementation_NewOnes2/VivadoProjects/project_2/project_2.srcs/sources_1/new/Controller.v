...
parameter JUMP_REG = 14, WRITE_RA1 = 15, WRITE_RA2 = 16;
...
always @(presentState, currentInstruction, OP)
    begin
        case(presentState)
            IDLE:
                begin
                    {Branch_EQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0000000;
            ...
            FETCH: // during FETCH, instruction is fetched and also new PC value is updated with PC+4
                begin
                    ...
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0010000;
            ...
            // and ALU source B is slected to sign extended +4 for Branch_EQ isntruciton
                begin
                    ...
                    Funct = 0;
                    if(OP==6'b000000 && currentInstruction[5:0]==6'b001000)
                        nextState = JUMP_REG;
            ...
            MEMREAD: // drung MEMREAD, the memory valu is read and stored in Data Register after 1 cycle
                begin
                    {Branch_EQ,  Branch_NEQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b1000000;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b000000;
                    Funct = 0;
            ...
            MEMWRITEBACK: // druing MEMWRITEBACK, the value of data register is written into memory and goes back to FETCH
                begin
                    {Branch_EQ,  Branch_NEQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0000011;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b000000;
                    Funct = 0;
                    nextState = FETCH;
                end
            MEMWRITE: // during MEMWRITE, the value of of write data is written intor the memory into memory
                begin
                    {Branch_EQ,  Branch_NEQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b1100000;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b000000;
                    Funct = 0;
                    nextState = FETCH;
                end
            ...
            ALUWRITEBACK: // during ALU write back, alu out is stored into reg
                begin
                    {Branch_EQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0000101;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b000000;
                    Funct = 0;
                    nextState = FETCH;
                end
            ...
            IMMEDIATEWRITEBACK: // wirte back into RF 
                begin
                    {Branch_EQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0000001;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b000000;
                    Funct = 0;
                    nextState = FETCH;
                end

            JUMP_REG: // add content of 25:21 with 20:16(which is always zero for branch register) and store in pc
                    // select pc source to be ALU resutls and write into pc
                begin
                    {Branch_EQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00001;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0000000;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b010000;
                    Funct = 0;
                    nextState = Branch_EQ_DUMMY;
                end
            WRITE_RA1: // for JAL, PC+4value is calulated and will be availeb at ALUOut after one cyclle
                begin
                    {Branch_EQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0000000;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b000100;
            ...
            WRITE_RA2:
                begin
                    {Branch_EQ,  Branch_NEQ, PCSrc, PCWrite} = 5'b00000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'b0001001;
                    {ALUSrcA, ALUSrcB, ALUOP} = 6'b000000;
                    Funct = 0;
                    nextState = JUMP;
                end
            default:
                begin
                    ...
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 7'bxxxxxxx;
