...
parameter EXECUTE = 7, ALUWRITEBACK = 8, BRANCH = 9, JUMP = 11, ADDIEXECUTE = 12, ADDIWRITEBACK=13;
    ...
    begin
        case(presentState)
            ...
            JUMP: // compute the new address directly by appending PC'S 4 bit with (immediate address * 4) and have a dummy state
                begin
                    {Branch, PCSrc, PCWrite} = 4'b0101;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 6'b000000;
                    {ALUSrcA, ALUSrcB, ALUOP} = 5'b00000;
                    nextState = BRANCH_DUMMY;
                end
            ADDIEXECUTE: // find the sum of A and intermiditate 16-bit constant
                begin
                    {Branch, PCSrc, PCWrite} = 4'b0000;
                    {IorD, MemWrite, IRWrite, RegDst, MemtoReg, RegWrite} = 6'b000000;
                    {ALUSrcA, ALUSrcB, ALUOP} = 5'b11000;
            ...
            ADDIWRITEBACK: // wirte back into RF 
                begin
                    {Branch, PCSrc, PCWrite} = 4'b0000;
