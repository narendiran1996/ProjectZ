...
\begin{tikzpicture}
    ...
    \node [state] at ([xshift=10em]JUMP) (ADDIEXECUTE) {\tiny ADDIEXECUTE};
    \node [state] at ([yshift=-7em]ADDIEXECUTE) (ADDIWRITEBACK) {\tiny ADDIWRITEBACK};

    \draw (DECODE.east) -| (ADDIEXECUTE.north)  node[pos=0.9,left] {\tiny OP == ADDI};
    \draw (ADDIEXECUTE.south) -- (ADDIWRITEBACK.north);
    \draw (ADDIWRITEBACK.south) |- ([yshift=-2em]MEMWRITEBACK.south) -- ++ (-4em,0) |- (FETCH.west);
